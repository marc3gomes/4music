...
namespace Blueskytechco\MenuBuilder\Block\Adminhtml;
...
class Widget extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    protected function _construct()
    {
        ...
        $this->_blockGroup = 'Blueskytechco_MenuBuilder';
        $this->_controller = 'adminhtml';
        $this->_mode = 'widget';
        $this->_headerText = __('Cms Block Insertion');

        $saveButtonClass = 'action-primary add-widget';
        $this->removeButton('back');
        if ($this->getRequest()->getParam('mode') === 'new') {
            $this->buttonList->update('save', 'label', __('Insert Cms Block'));
