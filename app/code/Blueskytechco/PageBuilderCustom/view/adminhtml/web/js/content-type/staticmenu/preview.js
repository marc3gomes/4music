  ...
  var Preview = /*#__PURE__*/function (_previewCollection2) {
    ...
    function Preview(contentType, config, observableUpdater) {
      ...
      _this.contentType.children.subscribe(function () {
        var sortableElement = (0, _jquery)(_this.wrapperElement).find(".staticmenu-container");
    ...
    _proto.bindEvents = function bindEvents() {
      ...
      _events.on("staticmenu:dropAfter", function (args) {
        if (args.id === _this2.contentType.id && _this2.contentType.children().length === 0) {
          _this2.addButton();
        }
      });

      _events.on("staticmenu:renderAfter", function (args) {
        if (args.contentType.id === _this2.contentType.id) {
          _this2.debouncedResizeHandler();
        }
      });

      _events.on("staticmenu-item:renderAfter", function (args) {
      ...
      _events.on("staticmenu-item:duplicateAfter", function (args) {
      ...
      _events.on("staticmenu-item:mountAfter", function (args) {
    ...
    /**
     * Add staticmenu-item to staticmenu children array
     */
    ;
    ...
    _proto.addButton = function addButton() {
      ...
      var createButtonItemPromise = (0, _contentTypeFactory)(_config.getContentTypeConfig("staticmenu-item"), this.contentType, this.contentType.stageId, {});
    ...
    /**
     * Get the sortable options for the staticmenu sorting
     *
    ...
    _proto.getSortableOptions = function getSortableOptions(orientation, tolerance) {
      ...
      return {
        handle: ".staticmenu-item-drag-handle",
    ...
    /**
     * Resize width of all child staticmenu. Dependently make them the same width if configured.
     */
    ;

    _proto.resizeChildButtons = function resizeChildButtons() {
      if (this.wrapperElement) {
        var buttonItems = (0, _jquery)(this.wrapperElement).find(".pagebuilder-staticmenu-item > a");
        ...
        if (this.contentType.dataStore.get("is_same_width") === "true") {
          if (buttonItems.length > 0) {
            ...
            var parentWrapperWidth = (0, _jquery)(this.wrapperElement).find(".staticmenu-container").width();
