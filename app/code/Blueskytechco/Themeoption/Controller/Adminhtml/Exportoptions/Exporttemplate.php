...
class Exporttemplate extends \Magento\Backend\App\Action
{
    ...
    {
        $fileName = 'elements_template.xml';
   
		$collection = $this->_objectManager->get('Magento\PageBuilder\Model\Template')->getCollection();
		$dom = $this->_parser->getDom();
		$dom->formatOutput = true;
		$root = $dom->createElement('root');
		$blocks = $dom->createElement('templates');
		$get_key = array('name', 'preview_image', 'template', 'meta_description', 'created_for');
		...
		$this->messageManager->addSuccess(__('Elements Template Exported.'));
