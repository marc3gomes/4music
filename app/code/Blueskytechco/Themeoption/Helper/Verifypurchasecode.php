...
class Verifypurchasecode extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function checkEnvatoPurchaseCode() {
        ...
        if($purchasecode != '' && base64_encode($purchasecode) == $purchasecode_confirm){
            return true;
        }
        return false;
    }
    ...
    public function removeEnvatoPurchaseCode() {
        ...
        if($purchasecode != ''){
           $re = $this->checkPurchaseCode($purchasecode, $get_domain, "r");
           return $re;
        }
        return 1;
    }
    ...
    public function checkPurchaseCode($purchasecode, $get_domain, $action) {
        ...
        $ch_verify = curl_init( $verify_url . '?item=38953490&purchasecode=' . $purchasecode.'&get_domain='.$get_domain.'&action='.$action);
