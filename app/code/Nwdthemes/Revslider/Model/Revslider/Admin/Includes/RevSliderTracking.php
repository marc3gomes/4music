...
class RevSliderTracking extends RevSliderFunctions {
	...
	public function __construct(){
		...
		if($this->is_enabled() === true){
			FA::add_filter('revslider_doing_html_export', array($this, 'count_html_export'), 10, 1);
			FA::add_filter('revslider_exportSlider_export_data', array($this, 'count_regular_exports'), 10, 1);
			FA::add_filter('revslider_retrieve_version_info_addition', array($this, 'add_additional_data'), 10, 1);
			FA::add_action('revslider-retrieve_version_info', array($this, '_run'), 10, 1);
		}
	}
	...
	public function get_tracking_data(){
		$data = FA::get_option('rs-tracking-data', array());

		return (!is_array($data)) ? array() : $data;
	}

	public function update_tracking_data($data){
		return FA::update_option('rs-tracking-data', $data);
	}

	public function delete_tracking_data(){
		return FA::delete_option('rs-tracking-data');
	}
	...
	public function get_unique_identifier(){
		$uid = FA::get_option('revslider-uid');
		if(strlen($uid) !== 12){
			$uid = substr(md5(mt_rand()), 0, 12);
			FA::update_option('revslider-uid', $uid);
		}
	...
	public function _run($update){
		...
		if(!isset($data['html_exports'])) $data['html_exports'] = 0;
		$data['licensed']	= $this->_truefalse(FA::get_option('revslider-valid', 'true'));
		$data['slider']		= array(
