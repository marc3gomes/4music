...
define(["jquery", "knockout", "mage/translate", "Magento_PageBuilder/js/events", "slick", "underscore", "Magento_PageBuilder/js/config", "Magento_PageBuilder/js/content-type-menu/hide-show-option", "Magento_PageBuilder/js/content-type/preview"], function (_jquery, _knockout, _translate, _events, _slick, _underscore, _config, _hideShowOption, _preview) {
  var Preview = /*#__PURE__*/function (_preview2) {
    ...
    function Preview(contentType, config, observableUpdater) {
      ...
      _this.instagramItemSelector = ".elementor-instagram-item";
      _this.centerModeClass = "center-mode";
      _this.messages = {
        EMPTY: (0, _translate)("Empty Instagram Photos."),
        NO_RESULTS: (0, _translate)("No instagram photos were found matching your condition."),
      ...
      _events.on("contentType:redrawAfter", function (args) {
        if (_this.element && _this.element.children) {
          var $element = (0, _jquery)(_this.element.children).find('.widget-instagram-slider');
    ...
    _proto.afterObservablesUpdated = function afterObservablesUpdated() {
      ...
      if (this.hasDataChanged(this.previousData, data)) {
        ...
        if (typeof data.image_format !== "string" || data.image_format === "") {
    ...
    _proto.initSlider = function initSlider() {
      if (this.element && this.appearance() === "carousel") {
        (0, _jquery)(this.element.children).find('.widget-instagram-slider').slick(this.buildSlickConfig());
      }
    };
    ...
    _proto.initGrid = function initGrid() {
      if (this.element && this.appearance() === "grid") {
        var $gridItems = (0, _jquery)(this.element.children).find('.col-instagram-item');
        var currentViewport = this.viewport();
        $gridItems.attr('class', '');
        $gridItems.attr('class', 'col-instagram-item col-'+this.getColBySetting(currentViewport));
      }
    };

    _proto.destroySlider = function destroySlider() {
      (0, _jquery)(this.element.children).find('.widget-instagram-slider').slick("unslick");
    };

    _proto.buildSlickConfig = function buildSlickConfig() {
      var attributes = this.data.main.attributes();
      var postCount = (0, _jquery)(this.widgetUnsanitizedHtml()).find(this.instagramItemSelector).length;
